...
def get_initialism_candidates(initialism:str, text_to_search:str, stopwords: List[str] | None = None) -> Set[str]:
    """
    ...
    if stopwords is None:
        stopwords = ['of', 'the', 'a', 'an', 'for', 'in', 'on', 'to', 'with', 'by', 'and']
    ...
    pattern = r'\b' + rf'\w*(?:\s+{stopword_pattern}){{0,2}}?\s+'.join([f'{char}\w*' for char in initialism])
